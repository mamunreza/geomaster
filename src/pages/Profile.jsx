    ...
    <div className="page">
      <div className="container" style={{ maxWidth: '720px' }}>
        ...
        <div className="card" style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), var(--secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem', fontWeight: 700, flexShrink: 0 }}>
        ...
        {/* Recent Games */}
        <div className="card">
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem' }}>📋 {t('profile.recentGames')}</h2>
          {(!user.scores || user.scores.length === 0) ? (
            <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '2rem' }}>{t('profile.noGames')}</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {user.scores.slice(0, 10).map((s, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', background: 'var(--bg-card2)', borderRadius: 'var(--radius-sm)', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <span style={{ fontSize: '1.2rem' }}>{s.gameType === 'Flags Quiz' ? '🏳️' : s.gameType === 'Capitals Quiz' ? '🏛️' : '🗺️'}</span>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{s.gameType}</div>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    ...
                    <span style={{ color: s.accuracy >= 80 ? 'var(--success)' : s.accuracy >= 50 ? 'var(--warning)' : 'var(--danger)', fontSize: '0.875rem' }}>🎯 {s.accuracy}%</span>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>✅ {s.correct}/{s.total}</span>
                  </div>
                </div>
              ))}
            </div>
